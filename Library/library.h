//
// Created by Yusuf Pisan on 3/26/18.
// Added Contributions by Connor Lauerman on 1/10/2021
//lists the mehtods and data that will be in the library object class
//

#ifndef ASS1_LIBRARY_H
#define ASS1_LIBRARY_H

#include <string>

// Only for class code, OK to use namespace
using namespace std;

// Library holds books - can add, remove and list books
class Library {
  // display all books in library
  friend ostream &operator<<(ostream &Out, const Library &Lib);

public:
 // constructor with default name 
 //inputs:
 // name: will name the libray, type string
  explicit Library(const string &Name);

  // destructor
  virtual ~Library();

  // add a new book
  // return true if successful, false if
  // book already in library
  bool addBook(const string &BookName);

  // remove a book
  // return true if successfully removed
  // false if book not in library
  bool removeBook(const string &BookName);

  // list all books
  void listAllBooks() const;

  // true if book found in library
  bool isInLibrary(const string &BookName) const;

  // gets the number of books in the library
  int getNumberOfBooks() const;

private:
  //number of books currently in library
  int numberOfBooks;

  //finds index location of book in library
  int findBook(const string &name) const;

  //data name of library
  string name;

  static const int MAX=100;

  //data array to carry books for library
  string books[MAX];
};


#endif  // ASS1_LIBRARY_H
